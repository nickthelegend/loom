...
const FR = path.join(DIR, "compositions", "pitch");
...
// The pad scene carries the LoomPad's own recorded audio, so it gets no VO
// line — vo:null means "no narration here, let the hardware talk".
const SCENES = [
  ["pain", 4.2, "01"],
  ["claim", 2.0, "02"],
  ["handoff", 3.6, "03"],
...
const call = (sel, s, atF, holdF) => {
  ...
  return `  gsap.set(${sel},{opacity:0,scale:0.94});\n` +
    `  tl.to(${sel},{opacity:1,scale:1,duration:0.20,ease:"back.out(3)"},${at});\n` +
    `  tl.to(${sel},{opacity:0,duration:0.16,ease:"power2.in"},${(at + hold).toFixed(2)});\n`;
};
const snap = (sel, s) =>
  `  gsap.set(${sel},{opacity:0,y:26});\n  tl.to(${sel},{opacity:1,y:0,duration:0.26,ease:"back.out(2.4)"},${(s * 0.06).toFixed(2)});\n`;
...
const S = {
  ...
  handoff: (s) => ({
    ...
    js: push("'#a'", s, 1.0, 1.10) + call("'#c1'", s, 0.08, 0.36) + call("'#t1'", s, 0.10, 0.34) +
  ...
  board: (s) => ({
    ...
    js: push("'#a'", s, 1.0, 1.09) + call("'#c1'", s, 0.08, 0.34) + call("'#t1'", s, 0.10, 0.32) +
        call("'#c2'", s, 0.50, 0.40) + call("'#t2'", s, 0.52, 0.38) + snap("'.band'", s),
  }),
  pad: (s) => ({
    // A short <video> never finishes decoding before the renderer captures it,
    // so the footage and its audio are composited over this window afterwards.
    // What stays here is the band, which must sit ON TOP of that footage.
    body: `<div class="clip bg" id="a" data-start="0" data-duration="${s}" data-track-index="1"></div>
      ${band("THE LOOMPAD", "it speaks the agent's reply back")}`,
    js: snap("'.band'", s),
  }),
  brain: (s) => ({
...
fs.writeFileSync(path.join(DIR, "index-pitch.html"), `<!doctype html>
...
${mounts.map((m) => `  <div class="clip" id="mt-${m.vo}" data-composition-id="pt-${m.id}" data-composition-src="compositions/pitch/${m.id}.html"
       data-start="${m.at.toFixed(2)}" data-duration="${m.secs}" data-track-index="1" style="inset:0;width:100%;height:100%"></div>`).join("\n")}
${mounts.filter((m)=>m.vo).map((m) => `  <audio class="clip" id="vo-${m.vo}" src="assets/vopitch/${m.vo}.wav"
