...
const FR = path.join(D, "compositions", "s30");
...
const SC = [
  ["pain", 4.2, "01", ["Every coding agent keeps its own memory."]],
  ["claim", 2.0, "02", ["Loom makes them one brain."]],
  ["stream", 6.4, "03", ["Pass the baton — the context goes with it.", "The next agent picks up mid-task."]],
  ["board", 5.4, "05", ["Every task carries the agent that owns it."]],
  ["pad", 5.0, null, ["We built the hardware too."]],
  ["brain", 4.6, "07", ["Four agents wrote this memory.", "All of them can read it."]],
  ["close", 2.6, "08", ["Loom. One brain, every agent."]],
];
...
fs.writeFileSync(path.join(D, "index-30.html"), `<!doctype html><html><head><meta charset="utf-8"><title>Loom</title>
<script src="https://cdn.jsdelivr.net/npm/gsap@3.12.5/dist/gsap.min.js"><\/script>
...
${M.map((m) => `  <div class="clip" id="mt-${m.id}" data-composition-id="t-${m.id}" data-composition-src="compositions/s30/${m.id}.html"
