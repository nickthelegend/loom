...
(async () => {
  ...
  mark("chat");
  await goClick("#box");
  await type("Add a --json flag to `loom agents` and update its test", 42);
  await sleep(700);
  await goClick("#cagent", 0, 520);          // open the agent switcher
  await sleep(900);
  await E(`(()=>{const m=document.getElementById('cmenu'); if(m) m.style.display='none';})()`);
  await goClick("#modelpick", 0, 520);        // and the model picker
  await sleep(1800);
  await E(`(()=>{const m=document.getElementById('cmenu'); if(m) m.style.display='none';})()`);
  await sleep(500);

  // ---------- 2. the board, and a task created on camera ----------
  mark("board");
  const [bx, by] = await at("button", await E(`[...document.querySelectorAll('button')].findIndex(b=>b.innerText.trim()==='Board')`));
  await glide(bx, by, 700); await click(bx, by);
  await sleep(2600);
  await E(`document.querySelectorAll('.scrim').forEach(s=>s.remove())`);
  await sleep(600);
  mark("newtask");
  try {
    await goClick("#addtask", 0, 700);
  } catch {
    const i = await E(`[...document.querySelectorAll('button')].findIndex(b=>/\\+\\s*task/i.test(b.innerText))`);
    if (i >= 0) { const [tx, ty] = await at("button", i); await glide(tx, ty, 700); await click(tx, ty); }
  }
  await sleep(1200);
  await type("Ship the phone brain-sheet fix", 46);
  await sleep(600);
  const ci = await E(`[...document.querySelectorAll('button')].findIndex(b=>/create|add|save/i.test(b.innerText.trim()))`);
  if (ci >= 0) { const [cx2, cy2] = await at("button", ci); await glide(cx2, cy2, 560); await click(cx2, cy2); }
  await sleep(2400);

  // ---------- 3. the brain, scrolled ----------
  mark("brain");
  const bi = await E(`[...document.querySelectorAll('button')].findIndex(b=>b.innerText.trim()==='Brain')`);
  const [rx, ry] = await at("button", bi); await glide(rx, ry, 700); await click(rx, ry);
  await sleep(3000);
  await E(`document.querySelectorAll('.scrim').forEach(s=>s.remove())`);
  await glide(1000, 620, 700);
  for (let i = 0; i < 22; i++) {
    await S("Input.dispatchMouseEvent", { type: "mouseWheel", x: 1000, y: 620, deltaX: 0, deltaY: 108 }, sid);
    await sleep(150);
  }
  await sleep(1200);
  mark("end");
